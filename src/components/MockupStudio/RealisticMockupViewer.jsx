...
export function RealisticMockupViewer({ isFullscreen, onToggleFullscreen }) {
  ...
  const handleTakeSnapshot = () => {
    ...
    if (svgElement && canvasElement) {
      ...
      ctx.fillStyle = '#183630';
      ctx.fillRect(0, 0, 1200, 1200);

      // Floor radial glow
      const grad = ctx.createRadialGradient(600, 750, 50, 600, 750, 450);
      grad.addColorStop(0, 'rgba(229, 198, 144, 0.25)');
      grad.addColorStop(1, 'rgba(24, 54, 48, 0)');
      ctx.fillStyle = grad;
  ...
  return (
    <div
      ...
      className="relative w-full h-full bg-[#183630] text-[#E5DAC9] flex flex-col justify-between select-none overflow-hidden print-grid-bg"
    >
      {/* =========================================================================
      ...
      <div className="flex items-center justify-between p-3.5 z-20 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#183630] border border-[#B8A98F]/40 text-xs shadow-md">
            <span className="w-2 h-2 rounded-full bg-[#E5C690] animate-pulse" />
            <span className="font-bold text-[#E5DAC9] text-[11px] sm:text-xs">
              {customizerProduct.name.split('(')[0]}
            </span>
            <span className="text-[#B8A98F]">•</span>
            <span className="text-[#E5C690] font-mono text-[10px] uppercase font-bold">
              {activeSide.toUpperCase()} VIEW
            </span>
          </div>

          <div className="hidden sm:flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#183630]/80 border border-[#B8A98F]/40 text-[10px] text-[#E5DAC9]/80">
            <Sun className="w-3 h-3 text-[#E5C690]" />
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-xl text-xs font-bold transition-all flex items-center gap-1 border ${
              showPrintBoundary
                ? 'bg-[#E5C690]/20 border-[#E5C690] text-[#E5C690]'
                : 'bg-[#183630] border border-[#B8A98F]/40 text-[#E5DAC9]/80 hover:text-[#E5DAC9]'
            }`}
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-xl bg-[#183630] hover:bg-[#183630]/80 border border-[#B8A98F]/40 text-[#E5DAC9] text-xs font-bold flex items-center gap-1 shadow-md transition-all"
            title="Export High-Res Snapshot"
          >
            <Camera className="w-3.5 h-3.5 text-[#E5C690]" />
      ...
      <div className="flex-1 w-full flex items-center justify-center relative min-h-0 overflow-hidden px-4">
        ...
        <div className="absolute w-[450px] sm:w-[550px] h-[450px] sm:h-[550px] bg-gradient-to-tr from-[#E5C690]/10 via-[#183630]/40 to-[#B8A98F]/10 rounded-full blur-3xl pointer-events-none" />
        ...
        {viewMode === '3d' ? (
          <div className="w-full h-full relative flex items-center justify-center">
            ...
            <button
              ...
              className="absolute top-4 right-4 px-3 py-1.5 rounded-xl bg-[#183630] border border-[#B8A98F]/40 text-[#E5DAC9] text-xs font-bold flex items-center gap-1.5 shadow-xl hover:scale-105 transition-all z-30"
            >
      ...
      <div className="flex items-center justify-between p-3.5 z-20 shrink-0">
        ...
        <div>
          {viewMode === '2d' && (
            <button
              ...
              className="px-4 py-2 rounded-2xl bg-[#E5C690] hover:bg-[#d9b87c] text-[#183630] text-xs font-black shadow-lg shadow-[#183630]/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 group"
            >
              <Rotate3d className="w-4 h-4 text-[#183630] group-hover:rotate-180 transition-transform duration-700" />
        ...
        <div className="flex items-center gap-1 p-1 rounded-2xl bg-[#183630]/90 border border-[#B8A98F]/40 shadow-xl backdrop-blur-md">
          <button
            onClick={handleZoomOut}
            className="w-7 h-7 rounded-xl hover:bg-[#E5DAC9]/10 text-[#E5DAC9]/80 hover:text-[#E5DAC9] flex items-center justify-center font-bold text-xs transition-colors"
            title="Zoom Out"
          >
          ...
          <span
            ...
            className="px-2 text-xs font-mono font-bold text-[#E5DAC9] cursor-pointer hover:text-[#E5C690] transition-colors"
