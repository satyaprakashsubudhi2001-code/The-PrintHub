...
export function ProductDetailView({ product, onBack, onSelectRelated }) {
  ...
  return (
    <div className="min-h-screen bg-[#080812] text-white pt-4 pb-28 sm:py-8 px-3 sm:px-8 select-none space-y-8 sm:space-y-12 animate-in fade-in w-full max-w-full overflow-x-hidden print-grid-bg">
      <div className="max-w-[1500px] mx-auto space-y-6 sm:space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="relative aspect-[4/3.5] rounded-3xl bg-[#101022] border border-white/10 overflow-hidden shadow-2xl group">
              <img
                src={activeImage}
                alt={product.name}
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-1.5 z-10">
                {product.badge && (
                  <span className="px-3 py-1 rounded-full text-xs font-black uppercase font-mono tracking-wider bg-[#101022]/90 text-[#06B6D4] border border-[#06B6D4]/30 shadow-md">
                    {product.badge}
                  </span>
                )}
                {discountPercent > 0 && (
                ...
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-2xl border-2 overflow-hidden shrink-0 transition-all ${
                      activeImage === img ? 'border-[#06B6D4] ring-2 ring-[#06B6D4]/30' : 'border-white/10 opacity-70 hover:opacity-100'
                    }`}
                  >
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-[#6C4DF6]/20 text-[#06B6D4] border border-[#06B6D4]/30 text-[10px] font-black uppercase font-mono">
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-slate-400">({product.reviewCount || '85'} customer reviews)</span>
                <span className="text-slate-600">•</span>
                <span className="text-emerald-400 font-bold font-mono">In Stock ({product.stock || 25} left)</span>
              </div>

              {/* Price Banner */}
              <div className="p-4 rounded-2xl bg-[#101022] border border-white/10 shadow-lg flex items-baseline justify-between">
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">Inclusive of all GST taxes • Free Shipping on ₹999+</span>
                </div>

                {discountPercent > 0 && (
                  <span className="px-2.5 py-1 rounded-xl bg-emerald-500/20 text-emerald-400 font-black text-xs font-mono border border-emerald-500/30">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300 flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400">{product.colors?.length} Colors Available</span>
              </label>
                ...
                {product.colors && product.colors.map((c, idx) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-all ${
                      selectedColor.name === c.name ? 'scale-110 border-white ring-2 ring-[#06B6D4] shadow-lg' : 'border-white/20 opacity-80 hover:opacity-100'
                    }`}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-[#06B6D4] font-bold text-[11px] cursor-pointer hover:underline">Size Guide</span>
              </div>
            ...
            {/* Quantity Stepper */}
            <div className="flex items-center justify-between p-3.5 rounded-2xl bg-[#101022] border border-white/10 text-xs shadow-lg">
              <div className="flex items-center gap-3">
                <span className="font-bold text-slate-300">Quantity:</span>
                <div className="flex items-center rounded-xl bg-[#080812] border border-white/10 p-1">
                  <button
                    type="button"
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="w-7 h-7 rounded-lg text-slate-400 hover:text-white flex items-center justify-center font-bold text-sm"
                  >
                    −
                  </button>
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#6C4DF6] via-[#2563EB] to-[#06B6D4] text-white font-black text-sm shadow-xl shadow-[#6C4DF6]/30 flex items-center justify-center gap-2 transition-all hover:scale-102 active:scale-98"
              >
              ...
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-[#101022] hover:bg-[#16162E] text-white font-bold text-xs border border-white/15 flex items-center justify-center gap-2 transition-all shadow-md"
              >
              ...
              {product.isCustomizable !== false && (
                <button
                  ...
                  className="w-full py-3.5 rounded-2xl bg-[#6C4DF6]/20 hover:bg-[#6C4DF6]/30 text-[#06B6D4] font-bold text-xs border border-[#06B6D4]/40 flex items-center justify-center gap-2 transition-all"
                >
            ...
            {/* Trust Points */}
            <div className="grid grid-cols-2 gap-3 text-xs text-slate-400 pt-2 border-t border-white/10">
              <div className="flex items-center gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="p-3.5 rounded-2xl bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-md"
            >
